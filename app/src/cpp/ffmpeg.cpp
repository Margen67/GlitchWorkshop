...
static void close_all_output_streams(OutputStream *ost, OSTFinished this_stream, OSTFinished others)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        *reinterpret_cast<int*>(&ost2->finished) |= (OSTFinished)(ost == ost2 ? this_stream : others);
    }
}
...
static void close_output_stream(OutputStream *ost)
{
    ...
    *reinterpret_cast<int*>(&ost->finished) |= (OSTFinished) ENCODER_FINISHED;
    if (of->shortest) {
...
void VideoProcessor::flush_encoders() {
    int i, ret;
...
static void (*frame_proc_ptr)(AVFrame*, enum AVPixelFormat) = NULL;
extern void set_frame_proc(void (*proc)(AVFrame*, enum AVPixelFormat));
extern void set_frame_proc(void (*proc)(AVFrame*, enum AVPixelFormat)) {
	frame_proc_ptr = proc;
}
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output, int eof)
{
	...
	if(frame_proc_ptr)
		frame_proc_ptr(decoded_frame, ist->dec_ctx->sw_pix_fmt);
    err = send_frame_to_filters(ist, decoded_frame);
...
int VideoProcessor::init_input_threads() {
...
VideoProcessor::VideoProcessor(size_t argc, char **argv) {
    init_dynload();
...
int VideoProcessor::transcode_init() {
...
int VideoProcessor::processMain() {
    int ti;
    current_time = ti = getutime();
    if (transcode() < 0)
        return 1;
