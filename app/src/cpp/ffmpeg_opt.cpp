...
{\
    ...
    for (i = 0; i < o->nb_ ## name; i++) {\
        ...
        if ((ret = check_stream_specifier(fmtctx, st, spec)) > 0)\
            outvar = (typeof(outvar))(o->name[i].u.type);\
        else if (ret < 0)\
            exit_program(1);\
    }\
}
...
static int read_ffserver_streams(OptionsContext *o, AVFormatContext *s, const char *filename)
{
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && !ost->stream_copy)
            ...
            choose_pixel_fmt(st, st->codec, codec, (AVPixelFormat)st->codecpar->format);
...
int VideoProcessor::init_complex_filters() {
    int i, ret = 0;
...
int VideoProcessor::configure_complex_filters() {
    int i, j, ret = 0;
...
int VideoProcessor::parse_options(size_t argc, char **argv) {
    OptionParseContext octx;
    uint8_t error[128];
    int ret;
