...
public class DataSourceSelectionActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent i) {
        if (resultCode == -1 && requestCode == IMAGE_OPENED_RESULT) {
            ...
            MainActivity.shuffleResources();
            Bitmap bmp = BitmapFactory.decodeFile(path);
            GlitchAlgo.setOriginalDataSrcBmp(new NativelyCachedBmp(bmp));

            if(MainActivity.originalBitmap != null) {
                Bitmap scaled = Bitmap.createScaledBitmap(bmp, MainActivity.originalBitmap.getW(), MainActivity.originalBitmap.getH(), false);
                GlitchAlgo.setResizedDataSrcBmp(new NativelyCachedBmp(scaled));
                if(scaled != bmp)
                    scaled.recycle();
            }
            MainActivity.restoreResources();
            bmp.recycle();
